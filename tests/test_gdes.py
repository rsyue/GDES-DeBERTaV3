...
class DebertaV3GDES(nn.Module):
    ...
    def forward_disc(
        ...
        # Cast disc_head to match hidden_states dtype at call time.
        # The backbone dtype may differ from float32 (e.g. after .half() or
        # autocast), and nn.Linear always initialises in float32, so we
        # synchronise here rather than at init where the final dtype isn't
        # settled yet.
        self.disc_head = self.disc_head.to(hidden_states.dtype)
